...
from ai_analyzer.utils.catalog_reader import (
...
from ai_analyzer.postprocessor.ai_description_writer import (
...
def run_single_table(
    ...
    dry_run: bool,
    run_id: int,
    process_descriptions: bool = False
):
    ...
    if analysis_type == "view_definition_analysis" and is_view:
        ...
        if not view_def:
            ...
            if dry_run:
                logging.info(f"[DRY RUN] Resultaat voor {table['table_name']}:\n{result}")
                return
            else:
        ...
        if process_descriptions:
            store_ai_table_description(run_id, table, result, author=author)
        return
...
def run_batch_tables(
    ...
    tables = get_tables_for_pattern_with_ids(server, database, schema, prefix)
    ...
    for table in tables:
        try:
            run_single_table(table, analysis_type, author, dry_run, run_id, process_descriptions)
