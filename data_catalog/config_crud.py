...
def fetch_dw_catalog_config_by_id(cfg_id: int):
    rows = q_all("""
        ...
        WHERE  cfg.id = :id
    """, {"id": cfg_id})
def set_dw_catalog_last_test_result(
    cfg_id: int,
    status: str | None,
    notes: str | None,
) -> None:
...
def reactivate_pbi_catalog_config(config_id: int, conn_id: int | None = None):
    ...
    sql = """
    update config.pbi_catalog_config
def set_pbi_catalog_last_test_result(
def reactivate_pbi_catalog_config(config_id: int, conn_id: int | None = None):
    params = {"id": config_id}
    sql = """
    update config.pbi_catalog_config
...
def deactivate_dl_catalog_config(config_id: int, conn_id: int | None = None):
    ...
    if conn_id is not None:
        sql += """
def set_dl_catalog_last_test_result(
    cfg_id: int,
    status: str | None,
    notes: str | None,
) -> None:
